...
export const updateTask = withAuth(
  async ({ supabase, user }, taskId: string, formData: FormData) => {
...
export const deleteTask = withAuth(async ({ supabase, user }, taskId: string) => {
...
export const getTasks = withAuth(
  async ({ supabase, user }, orderBy: "created_at" | "sort_order" = "sort_order") => {
...
export const getTaskById = withAuth(async ({ supabase, user }, taskId: string) => {
  const { data, error } = await supabase
    .from("tasks")
    .select("*")
    .eq("id", taskId)
 ...
 */
export const bulkUpdateTaskStatuses = withAuth(
  async (
    { supabase, user },

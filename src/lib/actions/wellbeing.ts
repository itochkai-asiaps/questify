...
export const createWellbeingEntry = withAuth(
  async (
    { supabase, user },
    moodScore: number,
    note?: string,
...
export const getTodaysLatestEntry = withAuth(
  async ({
    supabase,
    user,
  }): Promise<{
...
export const getWellbeingHistory = withAuth(
  async (
    { supabase, user },
    days: number = 7,
  ): Promise<{ data?: WellbeingEntry[]; error?: string }> => {
    ...
    if (!parsed.success)
      return { error: parsed.error.issues[0]?.message ?? "Invalid history data" };
    return { data: parsed.data };
  },
);

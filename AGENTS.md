...
| Неясные требования, ambiguities | `metis` | Pre-planning анализ: скрытые намерения, точки отказа |
...
- Если логический баг → был ли `hyperplan`?
...
**Визуальная фича:**
```
hyperplan → visual-engineering (вёрстка) → /visual-qa → Lefthook → commit → CI snapshots
```

**Логическая фича:**
```
hyperplan → ultrabrain/deep (реализация) → Lefthook → commit → CI tests
```

**Закрытие блока:**
```
/review-work → fix findings → Lefthook → commit → push
```

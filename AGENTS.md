...
**Все визуальные изменения проходят через агентов:**
1. `/frontend` — подбор цветов, шрифтов, стилей, вёрстка
2. `/visual-qa` — проверка контрастности, тёмной темы, pixel-perfect

**Порядок:** дизайн-задача → `/frontend` → реализация → `/visual-qa` → confirm.

...
- **5-retry stop**: если внешний агент (multimodal-looker, oracle, etc.) не отвечает 5 раз подряд — остановить цикл, не продолжать. Сообщить пользователю причину.
...
> Замерено через Playwright `page.goto({ waitUntil: 'load' })`. Это время HTML-загрузки, НЕ включает клиентский рендер данных (FCP выше). Худшие: формы создания (tasks/new, plans/new) — вероятно, тяжёлые клиентские компоненты.

### Дизайн-аудит — план анализа (3 фазы)

✅ **Фаза 1 — Инвентаризация (2026-07-13)**: 54 батча через `look_at`, 98.1% success. `multimodal-looker` (GPT-4o) — не сработал (не видит файлы по пути). Результаты в `.omo/tmp/ulw-20260713-design-audit.md`.

**Фаза 2 — Дизайн-система**: `visual-qa` dual-oracle на приоритетные экраны → Pass A (design-system integrity) + Pass B (visual fidelity + CJK).

**Фаза 3 — Синтез**:

1. Точечные фиксы багов
2. Карта дизайн-токенов (цвета, типографика, spacing, радиусы, тени)
3. Матрица переиспользования компонентов
4. Библиотека стилей (массовые апдейты, не постранично)
5. 2-3 визуальных прототипа для усиления текущего визуала
6. Roadmap внедрения дизайн-системы
7. Гипотеза: кастомные страницы в едином стиле с разными цветами

### Статус

- ✅ **Фаза 1 — завершена** (2026-07-13). 54 батча, 98.1% success rate. Инструмент: `look_at` (не `multimodal-looker` — тот не видит файлы по пути).
- 🔴 **CRITICAL**: 404 в светлой теме показывает форму логина вместо страницы 404
- 🔁 **Системные паттерны** (см. `.omo/tmp/ulw-20260713-design-audit.md`):
  1. Контраст текста в тёмной теме — 80%+ страниц
  2. Кнопки сливаются с фоном — 70%+
  3. Sidebar читаемость — 60%+
  4. Иконки тусклые — 50%+
  5. Placeholder-текст блёклый — 30%+
- ⬜ **Фаза 2-3**: на следующую сессию (дизайн-токены, матрица компонентов, багфиксы)
- ✅ Миграция 00015 на стейджинге (S v0.2.0-b255/00015)
- ✅ Скриншоты перенесены в `D:\projects\questify\screenshots\` (правило ARTIFACTS в AGENTS.md)
- 🔴 Канбан: 0 задач в колонках, дефолтные колонки не создаются
- 🔴 Tasks detail (mobile): деталь-панель задачи не отображается на 390px — клик по задаче не показывает детали (в отличие от десктопа)
- 🟡 `page:ready` — рекомендация добавить data-loaded атрибут для надёжного детекта загрузки

### Новый воркфлоу: инкрементальное дизайн-ревью

- **При работе со страницей**: `look_at` на затронутые экраны (dark+light) → баг-репорт
- **Не более 10 экранов** за сессию. Полный аудит — только разово или при редизайне.
- **В HANDOFF**: незавершённое дизайн-ревью → явная задача с `🔄 design-review: <pages>`
- **Инструмент**: `look_at` с `file_paths` (98% success, без rate limit). `multimodal-looker` (GPT-4o) — не использовать для файловых скриншотов (не видит пути).

### Скриншоты (перенесены в проект)

| Папка                                 | Файлов | Описание                   |
| ------------------------------------- | ------ | -------------------------- |
| `screenshots/skeletons/dark/`         | 17     | Тёмная тема, скелетоны     |
| `screenshots/skeletons/light/`        | 17     | Светлая тема, скелетоны    |
| `screenshots/loaded/dark/`            | 13     | Тёмная с данными, desktop  |
| `screenshots/loaded/light/`           | 13     | Светлая с данными, desktop |
| `screenshots/skeletons/mobile-dark/`  | 13     | Тёмная мобила, скелетоны   |
| `screenshots/skeletons/mobile-light/` | 9      | Светлая мобила, скелетоны  |
| `screenshots/loaded/mobile-dark/`     | 13     | Тёмная мобила, с данными   |
| `screenshots/loaded/mobile-light/`    | 13     | Светлая мобила, с данными  |

### Тестовый пользователь (стейджинг)

- Email: `test-1782723333911-rwcodw@example.com`
- Пароль: `TestPass123!`
- Данные: 31 задача (seed), 2 идеи, 1 план «Дизайн-аудит Questify» с 2 шагами
...
0da0ebf chore: dev tooling audit — Prettier, Gitleaks, env validation, Knip, bundle analyzer, Lighthouse CI
```
...
| --------------- | ----------------------------------------------------------------- |
| Коммитов        | 2                                                                 |
| fix:feat:chore  | 0:1:1 (K1 feat + tooling chore)                                   |
| Гейты           | vitest 285/285 ✅, tsc --noEmit ✅, lint 0 errors ✅, prettier ✅ |
| Файлов изменено | 27 (11 K1 + 16 tooling)                                           |

### Рекомендация на следующую сессию

1. 🔴 **Дизайн-аудит** — `design-consultant` + `visual-qa`. Скриншоты всех экранов, сверка на консистентность (+/- кнопки, чекбоксы, статус-бейджи различаются от экрана к экрану).
2. 🔴 **Дизайн-система** — унифицировать повторяющиеся элементы в `components/ui/`: badge статусов, +/- controls, checkbox-list, drag-handle.
3. 🟡 **Применить миграцию 00015** на стейджинг (`supabase db push`).
4. 🟡 **Запустить Knip** — `npx knip`, разобрать unused exports/dead code.
5. 🟡 **`codegraph init`** — проиндексировать кодовую базу (OpenCode session).
6. ⬜ K2 — Кастомные теги или Ручное QA по тест-плану
